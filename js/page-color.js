(function ($) {
  ...
  $(document).ready(function () {
    if (typeof carnavalsfPageColor !== 'undefined') {
      ['color1', 'color2'].forEach((colorKey, index) => {
        $('.carnavalsf-color-swatch input[value="color-' + (index + 1) + '"]')
          .siblings('.swatch')
          .css('background-color', carnavalsfPageColor[colorKey]);
      });
    }
  });
  ...
  function updateEditorClass() {
    ...
    if (editorIframe) {
      try {
        const editorWrapper = editorIframe.contentDocument?.querySelector('.editor-styles-wrapper');
        if (editorWrapper) {
